document.addEventListener('DOMContentLoaded', function () {
  ...
  // SENSOR POPUP + надёжное определение низа скролла
  (function () {
    ...
    // Находим реальный скроллер (если скролл не на wrapper, а глубже)
    ...
    function updateBottom() {
      ...
      // если нет прокрутки — класс не ставим
    ...
    function mountWatch() {
      ...
      // первая корректная отрисовка после открытия
  ...
  popupVideoPlay.addEventListener("click", (e) => {
    if (!video.contains(e.target) && !closeBtnVideo.contains(e.target)) {
      closePopup();
    }
  });
  document.addEventListener("keydown", (e) => {
