document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    const likeWrap = document.querySelector(".article_like-wrap");
    if (!likeWrap) return;
    const likeBtn = likeWrap.querySelector(".article_like-button");
    const dislikeBtn = likeWrap.querySelector(".article_dislike-button");
    const thanksBtn = likeWrap.querySelector(".article_thanks-btn");
    let timer = null;
    function activate() {
      likeWrap.classList.add("article-like");
      if (timer) clearTimeout(timer);
      timer = setTimeout(() => {
        likeWrap.classList.remove("article-like");
      }, 10000);
    }
    function deactivate() {
      likeWrap.classList.remove("article-like");
    ...
    if (likeBtn) likeBtn.addEventListener("click", activate);
    if (dislikeBtn) dislikeBtn.addEventListener("click", activate);
    if (thanksBtn) thanksBtn.addEventListener("click", deactivate);
