document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    const appSlider = new Swiper(sliderEl, {
      ...
      on: {
        ...
        setTranslate(swiper) { toggleNav(swiper); }, // тянем мышью/тачем — тоже обновляем
    ...
    function toggleNav(swiper) {
      ...
      // по ТЗ: показываем только на краях, в середине обе скрыты
      prev.style.visibility = 'hidden';
      next.style.visibility = 'hidden';

      if (atStart && !atEnd) {
        next.style.visibility = 'visible';   // только вперёд
      } else if (atEnd && !atStart) {
        prev.style.visibility = 'visible';   // только назад
      }
      // если и начало и конец одновременно — все слайды влезают, обе скрыты
    }
  })();
  ...
  (function () {
    ...
    if (!popup) return;
    const popupImg = popup.querySelector(".appmonisensa_popup-img");
    const closeBtn = popup.querySelector(".appmonisensa_popup-close");
    const content = popup.querySelector(".appmonisensa_popup-content");
    document.querySelectorAll(".appmonisensa_slider-item img").forEach(img => {
  ...
  (function () {
    ...
    const parent = document.querySelector(".appmonisensa_bottom");

    if (!text || !btn || !parent) return;

    const collapsedHeight = 175;
  ...
  (function () {
    const sidebar = document.querySelector(".appmonisensa_bottom-sidebar");
    const container = document.querySelector(".appmonisensa_bottom");
    const textBlock = document.querySelector(".appmonisensa_descript-text");
    if (!sidebar || !container || !textBlock) return;
    const topOffset = 80;
    function updateSidebar() {
      ...
      if (!textBlock.classList.contains("appmonisensa-expanded")) {
      ...
      if (
        scrollY + topOffset > containerTop &&
        scrollY + sidebarHeight + topOffset < containerBottom
      ) {
        sidebar.style.position = "fixed";
        sidebar.style.top = topOffset + "px";
        sidebar.style.left = containerRect.right - sidebarRect.width + "px";
        sidebar.style.right = "auto";
        sidebar.style.bottom = "auto";
      } else if (scrollY + sidebarHeight + topOffset >= containerBottom) {
        sidebar.style.position = "absolute";
